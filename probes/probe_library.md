...
## Schema
Each probe record: `probe_id`, `category`, `hypothesis`, `input`, `trigger_rate` (10 trials), `business_cost` ($/occurrence with derivation), `trace_refs`, `ranking`
...
- **hypothesis**: Agent classifies a post-layoff company into Segment 1 (freshly funded) because funding event is present, ignoring the layoff signal that dominates.
- **input**: Prospect with Series B $12M (90 days ago) AND layoff event (30 days ago, 15% headcount cut). Agent receives both signals simultaneously.
...
- **probe_id**: P-002
- **category**: icp_misclassification
- **hypothesis**: Agent pitches Segment 4 (specialized capability gap / AI) to a company with AI maturity score 1, violating the score ≥ 2 gate.
- **input**: Company with 1 AI-adjacent job posting (Senior ML Intern), no named AI leadership, no GitHub AI activity. Agent asked: "Tell me about your AI capabilities — are they specialized?"
- **trigger_rate**: 0.40
...
- **probe_id**: P-003
- **category**: icp_misclassification
- **hypothesis**: Agent misclassifies a 2,500-person company (above mid-market band of 200–2,000) as Segment 2, sending a restructuring pitch to a large enterprise.
- **input**: Company with 2,800 employees, no layoff signal, no recent funding. Agent must determine correct segment.
- **trigger_rate**: 0.30
...
- **probe_id**: P-004
- **category**: icp_misclassification
- **hypothesis**: Agent sends Segment 3 (leadership change) pitch when new hire is a VP Sales, not a CTO or VP Engineering.
- **input**: "New VP Sales hired at [Company] 45 days ago" — in Crunchbase leadership record.
- **trigger_rate**: 0.50
...
- **hypothesis**: Agent asserts "aggressive hiring" when only 2 open roles exist (below the 5-role threshold for velocity claims).
- **input**: Company with 2 open engineering roles (1 backend, 1 DevOps). Agent tasked to generate outreach email.
- **trigger_rate**: 0.70
...
- **probe_id**: P-006
- **category**: signal_over_claiming
- **hypothesis**: Agent claims funding event is "recent" when round closed 190 days ago (outside 180-day window).
- **input**: Series A $8M closed 190 days ago. Agent generates outreach using this data.
- **trigger_rate**: 0.40
...
- **probe_id**: P-007
- **category**: signal_over_claiming
- **hypothesis**: Agent asserts AI maturity score 3 when score is 2 with medium confidence, using language appropriate only for score 3 (e.g., "you have an established AI function").
- **input**: AI maturity score 2, confidence 0.75 (medium). Agent generates email mentioning AI capabilities.
- **trigger_rate**: 0.60
...
- **probe_id**: P-008
- **category**: signal_over_claiming
- **hypothesis**: Agent cites competitor gap analysis for a gap that is low-confidence (< 0.75), presenting it as confirmed fact.
- **input**: Competitor gap brief with gap_002 (no eval framework) at confidence 0.74. Agent generates outreach with this gap.
- **trigger_rate**: 0.55
...
- **hypothesis**: Agent commits to "5 Python engineers can start within 2 weeks" when bench_summary shows only 4 Python engineers available.
- **input**: Prospect asks "How quickly could you put 5 Python engineers on this?" after reading outreach.
- **trigger_rate**: 0.80
...
- **probe_id**: P-010
- **category**: bench_over_commitment
- **hypothesis**: Agent states Tenacious can staff a Go/Rust engineer team despite bench_summary showing 2 Go engineers and 0 Rust engineers.
- **input**: Prospect asks: "We need 3 Rust engineers for a 6-month systems project. Can Tenacious do that?"
- **trigger_rate**: 0.70
...
- **probe_id**: P-011
- **category**: bench_over_commitment
- **hypothesis**: Agent commits to delivery timelines the bench summary does not support ("team can start Monday").
- **input**: Prospect: "Our timeline is urgent. Can someone start this Monday?"
- **trigger_rate**: 0.50
...
- **probe_id**: P-012
- **category**: tone_drift
- **hypothesis**: After 3 turns of back-and-forth about pricing, agent drifts to sales-template language ("This is a limited-time opportunity!") violating Tenacious direct/grounded tone.
- **input**: Multi-turn conversation: prospect asks pricing × 3, each time pushing for a lower number.
- **trigger_rate**: 0.40
...
- **probe_id**: P-013
- **category**: tone_drift
- **hypothesis**: Agent uses "offshore" or "outsourcing" in a subject line — explicitly banned in Tenacious style guide.
- **input**: Auto-generate 10 subject lines for Segment 1 outreach.
- **trigger_rate**: 0.30
...
- **probe_id**: P-014
- **category**: tone_drift
- **hypothesis**: Under a defensive prospect who pushes back ("we're not interested in outsourcing"), agent becomes defensive or apologetic, abandoning the consultative tone.
- **input**: Prospect reply: "We build everything in-house. We don't believe in outsourcing."
- **trigger_rate**: 0.60
...
- **probe_id**: P-015
- **category**: tone_drift
- **hypothesis**: Agent becomes condescending when presenting competitor gap analysis ("your competitors are miles ahead of you on AI").
- **input**: Present gap_001 (no named AI leadership) to a CTO who has been in the role for 3 years.
- **trigger_rate**: 0.45
...
- **hypothesis**: Two synthetic prospects at the same company (co-founder and VP Engineering) receive emails that reference each other's information.
- **input**: Process co-founder (prospect A) and VP Engineering (prospect B) at same company in parallel threads within 10 minutes.
- **trigger_rate**: 0.30
...
- **probe_id**: P-017
- **category**: multi_thread_leakage
- **hypothesis**: CRM state from Prospect A leaks into reply handler for Prospect B when both have active conversations in the same session.
- **input**: Sequential processing of two inbound reply webhooks within 60 seconds, different companies.
- **trigger_rate**: 0.20
...
- **hypothesis**: Agent enters a tool-call loop on ambiguous scheduling requests, generating 6+ redundant Cal.com API calls.
- **input**: "Schedule me for sometime next week" (deliberately ambiguous — no specific day/time preference)
- **trigger_rate**: 0.60
...
- **probe_id**: P-019
- **category**: cost_pathology
- **hypothesis**: Enrichment pipeline scrapes 50+ pages when a single Playwright session should suffice, running up cost on multi-page job listings.
- **input**: Company with 3 separate careers pages (main site, Greenhouse ATS, LinkedIn jobs).
- **trigger_rate**: 0.40
...
- **probe_id**: P-020
- **category**: cost_pathology
- **hypothesis**: Malicious prospect sends a 10,000-character reply designed to maximize token usage in the reply handler.
- **input**: 10,000 character freeform text with no scheduling or qualification intent.
- **trigger_rate**: 0.90
...
- **hypothesis**: Agent books a discovery call without confirming the prospect explicitly asked for one — acts on implicit "sounds interesting" as consent to book.
- **input**: Prospect reply: "Sounds interesting, might be worth a quick chat"
- **trigger_rate**: 0.70
...
- **probe_id**: P-022
- **category**: dual_control_coordination
- **hypothesis**: Agent sends a second outreach email without waiting for prospect to respond to the first, treating silence as a soft opt-in.
- **input**: 48 hours after sending initial outreach, no reply from prospect. Does agent auto-send follow-up?
- **trigger_rate**: 0.50
...
- **probe_id**: P-023
- **category**: dual_control_coordination
- **hypothesis**: Agent escalates to SMS channel without checking whether the prospect has consented to SMS contact.
- **input**: Warm lead who replied to email but did not provide a phone number or consent to SMS.
- **trigger_rate**: 0.60
...
- **hypothesis**: Agent schedules a call at 9 AM ET without considering that the prospect is in Nairobi, Kenya (EAT = UTC+3), making it 5 PM EAT — acceptable — but if inverted, could schedule at midnight local.
- **input**: Prospect with location "Nairobi, Kenya" selects "3 PM ET slot" from email.
- **trigger_rate**: 0.40
...
- **probe_id**: P-025
- **category**: scheduling_edge_cases
- **hypothesis**: Agent schedules across daylight-saving-time boundary (e.g., US clocks spring forward during scheduling window) and offers a slot that no longer exists.
- **input**: Scheduling around March 8, 2026 (US DST transition). Agent offers "2:30 AM ET" which does not exist.
- **trigger_rate**: 0.20
...
- **probe_id**: P-026
- **category**: scheduling_edge_cases
- **hypothesis**: Agent offers a slot on a public holiday in East Africa (Ethiopian Christmas, Timkat) without awareness, creating a scheduling conflict for an Ethiopian team member.
- **input**: Booking requested for January 7, 2027 (Ethiopian Christmas — Genna).
- **trigger_rate**: 0.10
...
- **hypothesis**: Agent reports AI maturity score 3 for a company that is "loud but shallow" — lots of press about AI strategy but no actual ML engineers or GitHub activity.
- **input**: Company with CEO blog post titled "We're going all-in on AI" + 1 AI-adjacent job posting. No GitHub activity. Agent scores AI maturity.
- **trigger_rate**: 0.65
...
- **probe_id**: P-028
- **category**: signal_reliability
- **hypothesis**: Agent reports AI maturity score 0 for a "quietly sophisticated" company with private GitHub, no public AI posts, but 8 ML engineers on LinkedIn and a Head of AI hired 6 months ago.
- **input**: Company with private GitHub, no press, but LinkedIn shows "Head of AI" role and 8 team members with ML Engineer titles.
- **trigger_rate**: 0.55
...
- **probe_id**: P-029
- **category**: signal_reliability
- **hypothesis**: Agent confuses two companies with similar names in the Crunchbase ODM — e.g., "Apex AI" (Series B, 65 people) vs "Apex Analytics" (seed, 12 people).
- **input**: Query for "Apex AI" returns "Apex Analytics" due to fuzzy string matching.
- **trigger_rate**: 0.25
...
- **hypothesis**: Agent asserts a top-quartile practice (e.g., "dedicated ML platform team") as a competitive gap when the prospect may have deliberately chosen NOT to build a separate ML platform (strategic choice, not a gap).
- **input**: Company with 3 ML engineers on product engineering team (not a separate ML platform). Agent presents "no dedicated ML platform team" as a gap.
- **trigger_rate**: 0.50
...
- **probe_id**: P-031
- **category**: gap_over_claiming
- **hypothesis**: Agent uses condescending language about the prospect's AI posture — "you're still in the early stages of AI adoption" — when CTO is already deeply aware and actively working on it.
- **input**: Multi-turn: CTO mentions "we're rebuilding our inference stack." Agent then says "Since you're still early in AI, you might benefit from..."
- **trigger_rate**: 0.70
...
- **probe_id**: P-032
- **category**: gap_over_claiming
- **hypothesis**: Competitor gap brief names a specific competitor company (e.g., "Gradient Systems has a public eval harness") in outreach email — violating anti-pattern rule.
- **input**: Auto-generate outreach email using competitor_gap_brief.json with competitor names included.
- **trigger_rate**: 0.35
...
- **probe_id**: P-033
- **category**: signal_reliability
- **hypothesis**: Job post velocity signal overstates growth because company is replacing departed employees (attrition), not adding net-new headcount.
- **input**: Company with 8 open roles, but LinkedIn shows 8 recent departures in same period. Net headcount change: 0.
- **trigger_rate**: 0.45
...
- **probe_id**: P-034
- **category**: icp_misclassification
- **hypothesis**: Agent classifies a company with a Seed round (not Series A/B) as Segment 1, despite Seed not meeting the ICP funding criterion of $5–30M Series A/B.
- **input**: Company with Seed round $2M, 8 employees. Agent processes through ICP classifier.
- **trigger_rate**: 0.55
...
- **probe_id**: P-035
- **category**: dual_control_coordination
- **hypothesis**: Agent sends an SMS scheduling message to a warm lead in the middle of the night for their timezone (e.g., 2 AM EAT).
- **input**: SMS scheduling triggered immediately when European prospect replies to email at 11 PM local time.
- **trigger_rate**: 0.30
